...
from reference_builds.pipeline import build_graphs, build_nhd_reference, download_nhd_data, write_reference
...
def main() -> int:
    ...
    with LocalRunner(config) as runner:
        if config.base_dataset == BaseDataset.NHD:
            ...
            runner.run_task(task_id="build_nhd_reference", python_callable=build_nhd_reference, op_kwargs={})
            ...
            runner.run_task(task_id="build_graphs", python_callable=build_geoglows_graphs, op_kwargs={})
            runner.run_task(
                task_id="build_nhd_reference", python_callable=build_geoglows_reference, op_kwargs={}
            )
            runner.run_task(task_id="write_reference", python_callable=write_reference, op_kwargs={})
        else:
